...
**Pocket Casts catalog normalization for public-subscriber feeds** — PC's `add_feed_url` API behaves differently depending on whether it can reach the submitted URL. For auth-gated URLs (class `private-auth`) that PC's backend cannot fetch, PC creates a "user podcast" entry tied to that specific URL, preserving the subscriber feed as a separate subscription with its full episode archive. For publicly accessible URLs (classes `public-archive` and `public-subscriber`), PC recognises the feed, resolves it to a catalog UUID — equivalent to the iTunes `storeCollectionID` — and the resulting subscription points to the catalog entry rather than the submitted URL. If the catalog entry uses a different canonical URL (e.g. `https://www.nytimes.com/the-daily` for The Daily), the subscription will fetch episodes from whatever source PC uses for that catalog entry. In practice this means PC may only surface a limited episode window for that podcast (e.g. the most recent 50–100 episodes), regardless of how deep the subscriber archive is. Episodes in the subscriber archive that predate PC's catalog window cannot be marked played and will appear in the unmatched count at the end of the migration. The migration log's "retaining KVS URL to preserve access" note refers to which URL is exported for other destinations (Overcast, OPML); it does not guarantee PC will use that URL for its subscription.
