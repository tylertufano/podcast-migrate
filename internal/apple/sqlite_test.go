...
func setupSQLiteDB(t *testing.T) string {
	...
	insertPodcast := func(pk int, subscribed int, feedURL, title, author, imageURL interface{}) {
		t.Helper()
		_, err := db.Exec(
			`INSERT INTO ZMTPODCAST VALUES (?,?,?,?,?,?,NULL)`,
...
func TestSQLiteReader_MissingDurationColumn(t *testing.T) {
	...
	_, _ = db.Exec(`CREATE TABLE ZMTPODCAST (
		...
		ZFEEDURL TEXT, ZTITLE TEXT, ZAUTHOR TEXT, ZIMAGEURL TEXT, ZSTORECOLLECTIONID INTEGER)`)
	...
	_, _ = db.Exec(`INSERT INTO ZMTPODCAST VALUES (1, 1, 'https://feeds.example.com/show-a', 'Show A', 'Author', NULL, NULL)`)
...
func TestSQLiteReader_EpisodeFeedURL_CacheBusterStripped(t *testing.T) {
	...
	_, _ = db.Exec(`CREATE TABLE ZMTPODCAST (Z_PK INTEGER PRIMARY KEY, ZSUBSCRIBED INTEGER, ZFEEDURL TEXT, ZTITLE TEXT, ZAUTHOR TEXT, ZIMAGEURL TEXT, ZSTORECOLLECTIONID INTEGER)`)
	...
	_, _ = db.Exec(`INSERT INTO ZMTPODCAST VALUES (1, 1, 'https://podcast.example.com/feed.xml?t=1749316800000', 'Cache Buster Show', 'Author', NULL, NULL)`)
